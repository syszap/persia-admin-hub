...
const TopBar = ({ onToggleSidebar }: TopBarProps) => {
  ...
  const initial = user?.username?.charAt(0).toUpperCase() ?? 'ا';
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <Button
          variant="ghost"
          size="icon"
          className="relative h-9 w-9 rounded-xl hover:bg-muted/80 transition-colors"
          aria-label="اعلان‌ها"
        >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              className="gap-2 text-sm h-9 rounded-xl px-2 hover:bg-muted/80 transition-colors"
            >
          ...
          >
            ...
            <DropdownMenuItem className="rounded-lg h-10 text-sm cursor-pointer justify-end gap-2 px-3">
            ...
            <DropdownMenuItem
              ...
              className="text-destructive rounded-lg h-10 text-sm cursor-pointer justify-end gap-2 px-3 hover:bg-destructive/8 transition-colors"
            >
